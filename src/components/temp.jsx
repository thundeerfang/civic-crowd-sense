...
import { getComplaintImageUrl } from "../data/awsimage";
  ...
  try {
    ...
    const complaintsWithData = await Promise.all(
      rawComplaints.map(async (c) => {
        ...
        if (c.latitude && c.longitude) {
          try {
            const geoRes = await axios.get("https://nominatim.openstreetmap.org/reverse", {
              params: { lat: c.latitude, lon: c.longitude, format: "json", addressdetails: 1 },
              headers: { "User-Agent": "YourAppName/1.0" },
            });
            if (geoRes.data?.address) {
              const { postcode, city, state, suburb, town, village } = geoRes.data.address;
              address = `${suburb || town || village || ""}, ${city || town || village || ""}, ${state || ""} - ${postcode || ""}`;
            }
          } catch (err) {
        ...
        if (c.userId) {
          try {
            ...
            if (userDoc.exists()) {
              ...
              userName = userData.name || translations.noUser;
              phoneNumber = userData.phone || translations.noPhone;
            }
          } catch (err) {
        ...
        // 🔹 Fetch complaint image from AWS
        let imageUrl = null;
        if (c.userId && c.complaintId) {
          try {
            imageUrl = await withTimeout(getComplaintImageUrl(c.userId, c.complaintId), 3000);
          } catch (err) {
            console.error("Error fetching AWS image:", err.message);
            imageUrl = null;
          }
        }
